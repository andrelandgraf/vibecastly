...
- **Tag the environment off a branch-scoped var.** Because each Neon branch runs its own copy of the function, deriving `environment` from the injected `NEON_BRANCH` var keeps preview/branch errors separable from production in the Sentry dashboard.
